...
@st.cache_data
def load_data():
    # Try multiple possible locations for robustness (local + common deployment paths)
    possible_paths = [
        "data.json",
...
# Extra session state for feedback visibility
...
def reset_stage():
    """Reset for a new stage (fresh 5 random questions + reset attempts)."""
    st.session_state.current_question_index = 0
    st.session_state.stage_attempts = 0
    st.session_state.show_feedback = False
...
def next_stage():
    """Move to the next stage or end the module."""
...
def advance_question():
    """Advance to next question or next stage."""
...
# ====================== GAMEPLAY ======================
    ...
    # If game is over, show congratulations and stop
    ...
    # Safety check: make sure current_stage_index is valid
    if st.session_state.current_stage_index >= len(STAGES):
        st.session_state.game_over = True
        st.rerun()
    ...
    if len(st.session_state.current_questions) > 0:
        ...
        # Show question only if we are not showing feedback
